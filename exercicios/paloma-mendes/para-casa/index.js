...
function buscarTitulo(titulo, genero) {
  for (let filme of filmes) {
    let buscarTitulo = filmes.find(elemento => elemento.titulo.toLowerCase().includes(titulo.toLowerCase()));
...
function buscarGenero(genero) {
  for (let filme of filmes) {
    let buscarGenero = filmes.filter(elemento => elemento.genero.includes(genero.toLowerCase()));
    console.table(buscarGenero);
    break;
  }
}

buscarTitulo('Corpo Elétrico');
buscarTitulo('Homem-Aranha: Sem Volta para Casa');
buscarTitulo('ninh')
buscarGenero('sci-fi');
