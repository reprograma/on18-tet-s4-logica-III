let clienteLilit = {
  ...
  function calcularDescontoProduto(valor) {
    if(valor >= 200.0) {
  ...
  function calcularDescontoExtra(formaDePagamento, totalAPagar) {
    if(formaDePagamento == "pix" || formaDePagamento == "a vista") {
      let desconto = totalAPagar * 0.05
      valorFinal -= desconto
  ...
  function imprimirNota(quantidadeDeProdutos) {
    ...
    let notaFiscal = {
      ...
      'Valor Aplicado': `R$${descontoAplicado.toFixed(2).replace('.',',')}` ,
      'Valor à pagar': `R$${valorFinal.toFixed(2).replace('.',',')}`,
      'Data da compra': dataDaCompra.toLocalDateString('pt-BR',{
        hour: 'numeric',
        minute: 'numeric',
        second: 'numeric'
      })
  ...
  if(quantidadeDeProdutos > 10 || valorFinal > 800.0) {
    return console.table({
      ...notaFiscal,
      'Bônus': 'Você ganhou um CUPOM de R$50,00 para sua próxima compra!'
    })
  } else {
